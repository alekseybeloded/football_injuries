...
def test__get_players_for_team__renders_correct_template(admin_client, team):
...
@pytest.mark.django_db
def test__get_players_for_team__with_no_players(admin_client, team):
    response = admin_client.get(reverse('players', kwargs={'team_slug': team.slug}))
...
@pytest.mark.django_db
def test__get_players_for_team__redirects_for_anonymous_user(client, team):
...
@pytest.mark.django_db
def test__get_players_for_team__pagination(admin_client, team):
    for i in range(15):
